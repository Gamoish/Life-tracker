 ...
 * matching `isoWeekday` in `./date`). "Daily" is just all seven days
 * selected; "3 days a week" is any three — there is no separate cadence
 * branch, the schedule set is the only input that varies.
 *
 ...
 * exactly the same result the store would produce.
 *
 * Ported from the web app's `src/lib/habit-streak.ts` — the maths is
 * identical. The only thing that changed is where `today` comes from: this
 * app has no server clock to be careful about, `today()` in `./date` just
 * reads the device's own local date.
 */
